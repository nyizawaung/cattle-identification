...
class CATTLE_IDENTIFICATION():
    ...
    def Centroid_Tracking_PerCamera(self,mask,w,h,cam_counter,y1,y2):
        ...
        if cam_counter in self.CAM_TRACKER:
            ...
            if closet!=-1 :
                ...
                is_touching_top = self.is_touching_top(y1, cam_counter)
                self.update_tracking_mask_location(current_id,mask,cam_counter,current_point,is_touching_top,is_touching_bottom)
    ...
    def get_lowest_y(self,mask):
        try:
            ...
            if Ys.size == 0:
                return None
            max_y = np.max(Ys)
            xs_at_max_y = Xs[Ys == (max_y - 20)]
            #print("x1 : ",np.min(xs_at_max_y),"x2 : ",np.max(xs_at_max_y))
            if xs_at_max_y is None:
                return None
            return {
    ...
    def get_highest_y(self,mask):
        try:
            ...
            if Ys.size == 0:
                return None
            min_y = np.min(Ys)
            xs_at_min_y = Xs[Ys == (min_y + 20)]
            #print("x1: ",np.min(xs_at_min_y),"x2: ",np.max(xs_at_min_y))
            if xs_at_min_y is None:
                return None
            return {
    ...
    def is_touching_bottom(self,y2, cam_counter, threshold = 20):
        border_line = self.touching_border_threshold[cam_counter]
        #print(f"border line {border_line} for camera {cam_counter}")
        return False if border_line[1] == -1 else y2 >= border_line[1] - threshold #y2 590 border line = 600 - threshold

    def is_touching_top(self,y1, cam_counter, threshold = 20):
        border_line = self.touching_border_threshold[cam_counter]
        #print(f"border line {border_line} for camera {cam_counter}")
        return False if border_line[0] == -1 else y1 <= border_line[0] + threshold
    ...
    def update_tracking_mask_location(self, tracking_id, mask, cam_counter,current_point, is_touching_top, is_touching_bottom):
        ...
        for m in self.TRACKING_MASK_LOCATION:
            if m.tracking_id == tracking_id:
                #print("Updating tracking id:", tracking_id, "in cam ", m.cam_counter)
                ...
                if m.mask_area < m.last_mask_area:
    ...
    def find_matching_mask(self, current_mask,current_point, cam_counter,tracking_id,x1,y1,x2,y2,frame, threshold=0.1):
        ...
        is_overlapped = False
        mask_area_threshold = 0.8
        #if is_touching_top or is_touching_bottom:
            #print(f"{tracking_id} is touching_top : {is_touching_top}, is touching bottom : {is_touching_bottom}, cam_counter : {cam_counter}, cattle_position : {cattle_position}")    
        if check_camera in self.camera_lists:
            is_overlapped = self.camera_lists[check_camera]
            if is_overlapped:
                #mask_area_threshold = 1.5
                distance_threshold = 280
                mask_area_threshold = 1.7 if cattle_position == "MIDDLE" else 1.3 # not over lap in side area
            else:
                if cattle_position != "MIDDLE":
                    distance_threshold = 150
                    mask_area_threshold = 2.4
                else:
                    distance_threshold = 200
                    mask_area_threshold = 1.4
        #mask_area_threshold = 27000 # not overlapped
        #if is_overlapped:
        #    mask_area_threshold = 1.6
        #else :
        #    if cattle_position != "MIDDLE":
        #        mask_area_threshold = 2.4
            #else:
            #    mask_area_threshold = 27000
        ...
        if is_touching_bottom:
            ...
            #print(f"tracing id {tracking_id}'s line info is : ",line_info)
            ...
            for m in self.TRACKING_MASK_LOCATION:
                ...
                if m.cam_counter == target_counter and m.is_touching_top:
                    ...
                    if other_line is None:
                        continue
                    #print(f"tracing id {m.tracking_id}'s other line is : ",other_line)
                    isOverlapped, percentage = self.isOverlappedXs([line_info["x1"],line_info["x2"]], [other_line["x1"],other_line["x2"]],threshold)
                    distance = 1000
                    #print(f" tracking_id {tracking_id} and m.tracking_id {m.tracking_id} isOverlapped is : {percentage}")
                    if other_line is not None and isOverlapped:
                        ...
                        if distance < distance_threshold or (percentage > 0.5 and distance < distance_threshold +10 ): #by pass checking length for now
                        #    print(tracking_id ," centroid distance is : ",distance, " with tracking id ", m.tracking_id)
                            ...
                            #print(f"Overlapped {m.tracking_id} : {is_overlapped}, current mask area: {mask_area}, other mask area: {m.last_mask_area}, total area: {total_area}")
                            if total_area > mask_area_threshold * m.mask_area or total_area > max( 40000, m.mask_area * 1.3):
                                #print(f"{m.tracking_id} : mask_area_threshold : {mask_area_threshold} , m.mask_area : {m.mask_area}")
                            ...
                            if isSimilar is None or isSimilar:
                               #print(tracking_id," is Similar to ",m.tracking_id)
                               return m.tracking_id,is_touching_bottom,is_touching_top
            ...
            #print(f"tracing id {tracking_id}'s other line is : ",line_info)
            if line_info is None:
                return closed_tracking,is_touching_bottom,is_touching_top
            ...
            for m in self.TRACKING_MASK_LOCATION:
                if m.cam_counter == target_counter and m.is_touching_bottom:
                    ...
                    if other_line is not None and isOverlapped:
                        ...
                        distance = self.CALCULATE_EUCLIDEAN_DISTANCE(centroid,current_point) 
                        #print("centroid distance is : ",distance, " with tracking id ", m.tracking_id)
                        if distance < distance_threshold or (percentage > 0.5 and distance < distance_threshold + 10): # previously allow a threshold + 20
                            #print(tracking_id ," centroid distance is : ",distance, " with tracking id ", m.tracking_id)
                            mask_area = np.sum(current_mask)
                            ...
                            #print(f"Overlapped {m.tracking_id} : {is_overlapped}, current mask area: {mask_area}, other mask area: {other_mask_area}, total area: {total_area}")
                            if total_area > mask_area_threshold * m.mask_area and total_area < 38000: 
                            #    print(f"mask_area_threshold : {mask_area_threshold} , m.mask_area : {m.mask_area}")
                                continue
                            if percentage > 0.7 and distance < distance_threshold:
                                return m.tracking_id,is_touching_bottom,is_touching_top
                            isSimilar =self.Is_Similar(current_mask,m.tracking_id,x1,y1,x2,y2,frame)
            ...
            if True :#HOKKAIDO_ROI(x1,y1,x2,y2,h,w):  #only cow class
                ...
                if is_new:
                    ...
                    #prev_id[0] = self.addNewTracking(prev_id[1],w,h) #if not touching then insert itssssssssss
                        ...
                        #print("new tracking id : ",prev_id[0], " is touching  top ", is_touching_top, " is touching bottom ", is_touching_bottom)
                        info = MASKLOCATION(prev_id[0],masks_np[mask_count],cam_counter,prev_id[1],is_touching_top,is_touching_bottom,)
                        self.TRACKING_MASK_LOCATION.append(info)
                        print(f"new tracking id : {prev_id[0]}")
    ...
    def batch_frames(self,data, file_name, is_show_image, cap= None, tracking_cap = None, isFile = False, isCam = False):
        ...
        for vdo in data:
            for key in self.distortion_keys:
                ...
                if key in vdo:
                    ...
                    if key in self.touching_border_threshold_setting:
                        self.touching_border_threshold[video_counter] = self.touching_border_threshold_setting[key]
                    
                    break

            video_readers = cv2.VideoCapture(vdo)
            if(start_timer_adjustment[video_counter] > 0):
                vid_name = vdo.split('\\')[-4] + " _ " + vdo.split('\\')[-1]
                print(f" Adjusting vdo {vid_name} to start at {start_timer_adjustment[video_counter]}")
                fps = video_readers.get(cv2.CAP_PROP_FPS)  # Get FPS
                frame_to_skip = int(start_timer_adjustment[video_counter] * fps)
                video_readers.set(cv2.CAP_PROP_POS_FRAMES, frame_to_skip)
        ...
        ret = True
        skip_minute,skip_second = 2,0
        manual_skip = (skip_minute * 60 + skip_second)  * 20
        while ret:
            ...
            for video in videos:
                ...
                if not ret:
                    break
                if vid_counter in apply_correction_on and miss:
    ...
    def begin_process(self,cam_datas,isCam,camera_number=None,lane = "Left"):
        ...
        for idx in range(number_of_videos): 
            ...
            project = f'F:/Output/runs/{self.FARMNAME}_identification/parallel_tracking_identification/July Report 2025/Straighten_Sumiyoshi-44/{model_name}/_{channel}'
    ...
    def setup_distortion_settings_straight():
        setting5_52 = CorrectDistortedImage().createSetting(k1=-0.9,k2=1,k3=-0.49,p1=0,p2=-0.007,focal_length_x=1.14,focal_length_y=1.01,center_x=0.52,center_y=0.43,scale=1.08,aspect=0.78)    
        setting5_53 = CorrectDistortedImage().createSetting(k1=-0.9,k2=1,k3=-0.49,p1=0,p2=-0.007,focal_length_x=1.18,focal_length_y=0.97,center_x=0.54,center_y=0.42,scale=1.09,aspect=0.78)        
        setting5_54 = CorrectDistortedImage().createSetting(k1=-0.9,k2=1,k3=-0.49,p1=0,p2=-0.007,focal_length_x=1.18,focal_length_y=0.97,center_x=0.53,center_y=0.42,scale=1.11,aspect=0.78)
        setting5_51 = CorrectDistortedImage().createSetting(k1=-0.9,k2=0.98,k3=-0.49,p1=0,p2=-0.008,focal_length_x=1.1,focal_length_y=1.01,center_x=0.52,center_y=0.5,scale=1.07,aspect=0.78)
 
        setting6_03 = CorrectDistortedImage().createSetting(k1=-0.12,k2=0.17,k3=-0.05,p1=0.007,p2=0.008,focal_length_x=1.39,focal_length_y=0.64,center_x=0.53,center_y=0.53,scale=1.01,aspect=1.0)
        setting003 = CorrectDistortedImage().createSetting(k1=1.0,k2=0.0,k3=0.0,p1=-0.042,p2=-0.069,focal_length_x=4.71,focal_length_y=1.72,center_x=0.72,center_y=0.73,scale=1.02,aspect=1.64)
        setting004 = CorrectDistortedImage().createSetting(k1=-0.03,k2=0.29,k3=-0.14,p1=-0.041,p2=-0.0,focal_length_x=0.85,focal_length_y=0.81,center_x=0.51,center_y=0.54,scale=1.01,aspect=1.0)
        settings = {}
        keys = ["5-52","5-53","5-54","5-51","-6-03"," 003"," 004"]
        settings['5-52'] = setting5_52
        settings['5-53'] = setting5_53
        settings['5-54'] = setting5_54
        settings['5-51'] = setting5_51
        settings['-6-03'] = setting6_03
        settings[' 003'] = setting003
        settings[' 004'] = setting004
        return keys,settings
    ...
    def setup_touching_border_threshold_settings():
        touching_border_threshold = {}
        padding = 15
        touching_border_threshold['5-51'] = (45,-1) #top, bottom (-1 is ignore) 
        touching_border_threshold['5-52'] = (35 , 596) 
        touching_border_threshold['5-53'] = (31, 603) 
        touching_border_threshold['5-54'] = ( -1, 613) 
        return touching_border_threshold
    #region parameters
    #region Tracker
    STORED_IDS =[]
    STORED_MISS = []
...
#models\December 2024\Sumiyoshi_dec16_2024_10000_iters_v9
    ...
    touching_border_threshold_setting = setup_touching_border_threshold_settings()
    camera_lists = {}
    touching_border_threshold = {}
...
if __name__ == '__main__':
    ...
    [
       ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-21\\1020"], 
       ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-21\\1020"],
       ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-21\\1020"],
       ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-21\\1020"]
    ],

    # [
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-21\\1005"], 
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-21\\1005"],
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-21\\1005"],
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-21\\1005"]
    # ],

    
    # [
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-21\\1505"], 
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-21\\1505"],
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-21\\1505"],
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-21\\1505"]
    # ],
    # [
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-21\\01"], 
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-21\\01"],
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-21\\01"],
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-21\\01"]
    # ],
    # [
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-21\\05"], 
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-21\\05"],
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-21\\05"],
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-21\\05"]
    # ],
    #     [
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-21\\11"], 
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-21\\11"],
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-21\\11"],
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-21\\11"]
    # ],

    # [
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-21\\15"], 
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-21\\15"],
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-21\\15"],
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-21\\15"]
    # ],

    # [
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-21\\16"], 
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-21\\16"],
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-21\\16"],
    #    ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-21\\16"]
    # ],

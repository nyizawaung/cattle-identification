...
##==== EF VECTOR MODEL DEFINITION ======
class EffNetEmbedding(nn.Module):
    def __init__(self, backbone_name, num_classes, emb_dim):
        super().__init__()
...
##==== EF VECTOR MODEL DEFINITION ======
class ProtoIdentifier:
    def __init__(self, embedder, prototypes, proto_labels, tau, device):
    ...
    @torch.no_grad()
    def predict_stack(self, images_tensors: List[torch.Tensor], tau=None):
        if tau is None:
            tau = self.tau
        if len(images_tensors) == 0:
            return Counter()

        x = torch.stack(images_tensors).to(self.device)
        emb = self.embedder.get_embedding(x)
        sims = emb @ self.prototypes.T         # cosine similarity
...
class CATTLE_IDENTIFICATION(PipelineMixin):
    def load_detector():
        ...
        detection_model = 'models\\KNP Night December 2025\Base_rtx8000_03_December_2025_20000_v2/model_final.pth'
        ...
        cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.55 #0.6
    ...
    def draw_bounding_box(self,image, box, label,is_recheck_periods,tracking_id,font_scale = 2,color =(0, 255, 0)):
    ...
    def Get_TrackingBatch_Default(self,isReset=False):
        ...
        default = {'GT': [GT], 'COUNT': [0], 'IS_STABLE': False, 'HAS_MISSED_FRAME': False,
                   'REIDENTIFY_MISSED_COUNT': self.REIDENTIFY_MISSED_COUNT, 'RECHECK_COUNT':0 }
        #'POSTURE': posture} consider adding posture in data collection
        return default
    ...
    def need_identification_due_to_standing(self, track_index, is_outside_gap, tracking_id):
        if is_outside_gap and not self.BATCH_CLASSIFICATION_TRACKER[track_index]['STANDING_IDENTIFICATION'] and \
           not self.BATCH_CLASSIFICATION_TRACKER[track_index]['PREVIOUS_MAX_PREDICTED_ID'] in [None,'Identifying']: #if never of standing then do it
    ...
    def GetBatchPredictedId(self, tracking_id, is_small, save_dir, missed_frame_count, location, w, h, is_outside_gap):
        ...
        if missed_frame_count > 5:
            has_missed_frame = True
            # print(f'{tracking_id} missed {missed_frame_count} frame and need to reidentify')# missed more than 5 frames then re-identify for 10 frames
        # default = {'GT': ['Identifying'], 'COUNT': [0], 'IS_STABLE': False, 'HAS_MISSED_FRAME': has_missed_frame,
        #            'REIDENTIFY_MISSED_COUNT': self.REIDENTIFY_MISSED_COUNT, 'RECHECK_COUNT':0, 'POSTURE': []}
        FORCE_IDENTIFICATION = False
        ...
        if tracking_id in self.TRACKER:  # already have tracking record
            ...
            track_index = self.TRACKER.index(tracking_id)
            #posture = self.CLASSIFICATION_TRACKER[track_index]['POSTURE']
            ...
            #posture = self.get_cattle_posture(mask)
        ...
        #if not self.BATCH_CLASSIFICATION_TRACKER[track_index]['STANDING_IDENTIFICATION']:
        #    is_inside_standing_area = self.is_inside_standing_area(location, w, h)
        # store missed_frame :
        total_predictions = 0
        if not is_small:
        ...
        if False and IS_STABLE: #no more is stable check
            maxPredictedId = self.BATCH_CLASSIFICATION_TRACKER[track_index]['HOLDING']
            return maxPredictedId, tracking_id

        if total_predictions > self.total_predictions  and not self.CLASSIFICATION_TRACKER[track_index][
            'HAS_MISSED_FRAME'] and not FORCE_MISSED_IDENTIFICATION and not IS_RECHECK_PERIOD:
        #and not IS_STABLE:  # and if not stable
            #is_moving = False
            ...
            if FORCE_MISSED_IDENTIFICATION:  # force missed identification
               # print(f"counter {maxCounters}, previous_max_predicted_id : {previous_max_predicted_id}")
                ...
                if maxCounters is None or len(maxCounters) == 0:
                    ...
                    maxPredictedId = maxCounters[0][0]
                #maxPredictedId = "9999" #force testing
                #print(
                ...
                # elif previous_max_predicted_id != None and previous_max_predicted_id != 'Reidentifying' and previous_max_predicted_id != 'Identifying': #if not same then update  due to tracking conflict
                    ...
                    #return self.UpdateTrackingIDAndBatchInfo(tracking_id, location, maxPredictedId, w, h, IS_RECHECK_PERIOD)
    ...
    def isOutsideofGap(self,center,isTouching,area = 50000):
        # is outside of ROI, >= for inside
        x, y = center
        if self.GAP_ROI is None:
            return  area > 20000 and not isTouching and x > 100 and x < self.save_width - 150 and y > 150 and y < self.save_height - 150 # update with variable
        if cv2.pointPolygonTest(self.GAP_ROI, center, False)  >= 0 or area < 20000:
            return False
        
        if not isTouching and x > 100 and x < self.save_width - 150 and y > 150 and y < self.save_height - 150 and area > 20000: #not close to border
            return True

        return not isTouching
    ...
    def CoreProcess(self,boxes,masks_np,h,w,frame,cam_counter,frame_count):
        ...
        #cv2.polylines(frame, [self.GAP_ROI],   True, (255,0,255), 2)
            ...
            if True :#HOKKAIDO_ROI(x1,y1,x2,y2,h,w):  #only cow class
                ...
                if tracking_id != -1:
                    if int(tracking_id) in self.STORED_IDS:
                        ...
                        idx = self.STORED_IDS.index(int(tracking_id))
                        #self.update_tracking_mask_location(tracking_id,masks_np[mask_count],cam_counter,prev_id[1],is_touching_top,is_touching_bottom) # no need for single
                ...
                if is_new:
                    ...
                    if isSmall and self.isHumanRatio(x2-x1,y2-y1,area) : #prioritize the size first
                        ...
                        continue
                    if isInMiddle and area < 11000 and not is_touching_right:
                        #print("I am middle : and small with area", area )
                        continue
                ...
                #predicted_id
                is_outside_gap = self.isOutsideofGap(prev_id[1], isTouching=isSmall,area = area)
                #cv2.circle(image, center_coordinate, radius=3, color=color, thickness=-1)
                #print("Ceter is ",prev_id[1], " is outside gap ", is_outside_gap)
                #cv2.circle(frame, (int(prev_id[1][0]),int(prev_id[1][1])), 5, (0, 255, 255), -1)
    ...
    def get_cattle_posture(self,mask: np.ndarray):
        ...
        # Map to 8 directions (45° each)
        # 0° = facing right (OpenCV defines angle from x-axis)
        # Adjust mapping to your camera orientation if needed
        directions = {
            (337.5, 360): 1,   # top
            (0, 22.5): 1,      # top
    ...
    def begin_process(self,cam_datas,isCam,camera_number=None,lane = "Left"):
        ...
        for idx in range(number_of_videos): 
            ...
            project = f'E:/Output/runs/{self.FARMNAME}_identification/EF_vector 8 Jan 2026/KNP/{model_name}/{channel}'
    ...
    model_path ="models\\Side Lane August 2025\\Base_rtx8000_December_2025_20000_v2" #Sumiyoshi_dec16_v3_human_2024_10000_iters_v6
    #print("initializing under main")
    #similarity_checker = CheckSimilarity()
    class_names = []
    
    ############### Identification Model Loading #########################
    #predictor_path = Path("identification_models_KNP") / "KNP_EFFNETV2_Vector"
    SAVE_ROOT = Path("identification_models_KNP") / "KNP_EFFNETV2_Vector"
    MODEL_PATH = SAVE_ROOT / "best_model.pth"
    PROTO_PATH = SAVE_ROOT / "prototypes.pt"
    MAP_PATH   = SAVE_ROOT / "class_mappings.json"
    EMBED_DIM = 512    
    ...
    embedder = EffNetEmbedding(MODEL_NAME, NUM_CLASSES, EMBED_DIM).to(device)
    ...
    #embedder = EffNetEmbedding(MODEL_NAME).to(device)
    ...
    proto_pack = torch.load(PROTO_PATH, map_location=device)

    prototypes   = proto_pack["prototypes"].to(device)   # [K, D]
    proto_labels = proto_pack["proto_labels"]             # List[str]
    tau = 0.35

    print(f"✅ Loaded {len(proto_labels)} prototypes | τ={tau}")

    infer = ProtoIdentifier(
        embedder=embedder,
...
if __name__ == '__main__':
    ...
    inputs = [
            ...
            #     # ["E:\\E2\\Ch002_Camera 002_E2\\2025-09-20\\0514"],
            # ],
            # [
            #      ["H:\\Nyi Zaw Aung\\815_CowDataChecking\\KNP\\Standard_Format\\ALL_Channel_One_Day\\Ch001_Camera 001_E2\\2025-12-24\\0023"]
            # ],           
            # [
            #      ["H:\\Nyi Zaw Aung\\815_CowDataChecking\\KNP\\Standard_Format\\ALL_Channel_One_Day\\Ch002_Camera 002_E2\\2025-12-24\\0023"]
            # ],
            # [
            #      ["H:\\Nyi Zaw Aung\\815_CowDataChecking\\KNP\\Standard_Format\\ALL_Channel_One_Day\\Ch003_Camera 003_E2\\2025-12-24\\0023"]
            # ],
            # [
            #      ["H:\\Nyi Zaw Aung\\815_CowDataChecking\\KNP\\Standard_Format\\ALL_Channel_One_Day\\Ch004_Camera 004_E2\\2025-12-24\\0023"]
            # ],
            
            [
             
                 ["H:\\Nyi Zaw Aung\\815_CowDataChecking\\KNP\\Standard_Format\\ALL_Channel_One_Day\\Ch005_Camera 005_E2\\2025-12-24\\0023"]
            ],
            
            [
                 ["H:\\Nyi Zaw Aung\\815_CowDataChecking\\KNP\\Standard_Format\\ALL_Channel_One_Day\\Ch006_Camera 006_E2\\2025-12-24\\0023"]
            ],
            [
                 ["H:\\Nyi Zaw Aung\\815_CowDataChecking\\KNP\\Standard_Format\\ALL_Channel_One_Day\\Ch007_Camera 007_E2\\2025-12-24\\0023"]
            ],
           
            ## xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx ## xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx ##
            # [
            #      ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\KNP\\Standard_Format\\ALL_Channel_One_Day\\Ch002_KP-FHDD-3-02\\2025-12-17\\0023"]
            # ],
            # [
             
            #      ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\KNP\\Standard_Format\\ALL_Channel_One_Day\\Ch005_KP-FHDD-4-05\\2025-12-17\\0023"]
            # ],
            # [
            
            #      ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\KNP\\Standard_Format\\ALL_Channel_One_Day\\Ch006_KP-FHDD-4-06\\2025-10-17\\0023"]
            # ],
            # [
             
            #      ["G:\\Nyi Zaw Aung\\815_CowDataChecking\\KNP\\Standard_Format\\ALL_Channel_One_Day\\Ch003_Camera 003_E2\\2025-04-17\\0023"]
            # ],
            # [
            ...
            #      ["G:\\Nyi Zaw Aung\\815_CowDataChecking\\KNP\\Standard_Format\\ALL_Channel_One_Day\\Ch006_Camera 006_E2\\2025-04-16\\0023"]
            # ],
            
            # [
            #      ["G:\\Nyi Zaw Aung\\815_CowDataChecking\\KNP\\Standard_Format\\ALL_Channel_One_Day\\Ch005_Camera 005_42\\2025-04-16\\0023"]
            # ],
            # [
            #      ["G:\\Nyi Zaw Aung\\815_CowDataChecking\\KNP\\Standard_Format\\ALL_Channel_One_Day\\Ch006_Camera 006_42\\2025-04-16\\0023"]
            # ],
        #    [
             
        #          ["G:\\Nyi Zaw Aung\\815_CowDataChecking\\KNP\\Standard_Format\\ALL_Channel_One_Day\\Ch004_Camera 004_E2\\2025-04-16\\0023"]
        #     ],
        #     [
        ...
        #   [
        #          ["G:\\Nyi Zaw Aung\\815_CowDataChecking\\KNP\\Standard_Format\\ALL_Channel_One_Day\\Ch001_Camera 001_E2\\2025-04-14\\002305"]
        #     ]
    ...
    for allCams in inputs:
        ...
        #read first frame from allCams
        first_frame = None
        for camPath in allCams:
            #get first video file from the cam folder
        ...
        begin_identification.GAP_ROI = sleeper_divider.get_gap_roi(first_frame)

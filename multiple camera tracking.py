...
class CheckSimilarity():
    ...
    def extract_embedding(self,img: Image.Image) -> np.ndarray:
        ...
        with torch.no_grad():
            embedding = self.effnet_model(tensor)
...
class CATTLE_IDENTIFICATION():
    def load_detector():
        ...
        detection_model = 'models\\Base-sidelane July 2025\\Base_rtx8000_June_10000_v1/model_final.pth'
        ...
        cfg.MODEL.ROI_HEADS.NMS_THRESH_TEST = 0.45 #0.3
    ...
    def Centroid_Tracking_PerCamera(self,mask,w,h,cam_counter,y1,y2,target_camera):
        ...
        current_point = self.find_mask_center(mask,cam_counter)
        least_missed_frame = 100
        for data in range(len(self.CAM_TRACKER[target_camera])):
            distance = self.CALCULATE_EUCLIDEAN_DISTANCE(data[1],current_point)
    ...
    def is_touching_bottom(self,y2, threshold=20):
        return abs(y2 - (self.image_height - 1)) <= threshold

    def is_touching_top(self,y1, threshold=20):
        return y1 <= threshold

    def update_tracking_mask_location(self, tracking_id, mask, cam_counter,current_point, is_touching_top, is_touching_bottom):
        
        for m in self.TRACKING_MASK_LOCATION:
            #print("Updating tracking id:", m.tracking_id, "with cam counter")
    ...
    def isOverlappedXs(self,first, second, threshold = 0.5):
        first_x1 = first[0]
        first_x2 = first[1]
        second_x1 = second[0]
        second_x2 = second[1]
    ...
    def Is_Similar(self,mask,tracking_id,x1,y1,x2,y2,frame):
        ...
        if score > 0.6:
            ...
            return False
            print("❌ No confident match.") #print(f"Deleting tracking {self.STORED_IDS[i-removed]} due to missed count {missed}")
    ...
    def find_matching_mask(self, current_mask,current_point, cam_counter,tracking_id,x1,y1,x2,y2,frame, threshold=0.1):
        ...
        is_overlapped = False
        if check_camera in self.camera_lists:
            is_overlapped = self.camera_lists[check_camera]
            if is_overlapped:
                distance_threshold = 250
            else:
                distance_threshold = 170
        print(tracking_id," is_overlapped is : ",is_overlapped, " in camera ", check_camera)
        #print(is_touching_bottom, ' is touching bottom')
        #print(is_touching_top, ' is touching top')
        #for masklocation in self.TRACKING_MASK_LOCATION:
        #    print(f'{masklocation.tracking_id}  is tracking id, cam_counter {masklocation.cam_counter}, is_touching_top {masklocation.is_touching_top}, is_touching_bottom {masklocation.is_touching_bottom}')
        #print(closed_tracking, ' tracking : is touching top: ', is_touching_top, ' is touching bottom: ', is_touching_bottom)
            ...
            for m in self.TRACKING_MASK_LOCATION:
                ...
                if m.cam_counter == target_counter and m.is_touching_top:
                    ...
                    if other_line is not None and isOverlapped:
                        ...
                        distance = self.CALCULATE_EUCLIDEAN_DISTANCE(centroid,current_point) 

                        print(tracking_id ," centroid distance is : ",distance, " with tracking id ", m.tracking_id)
                        if distance < distance_threshold or (percentage > 0.5 and distance < distance_threshold + 20 ): #by pass checking length for now
                            mask_area = np.sum(current_mask)
                            other_mask_area = np.sum(m.mask)
                            total_area = mask_area + other_mask_area
                            if (not is_overlapped and total_area > 27000) or (is_overlapped and total_area > 38000) : #compare mask area
                                print(f"Overlapped : {is_overlapped}, current mask area: {mask_area}, other mask area: {other_mask_area}, total area: {total_area}")
                                continue
                            print(f"Overlapped : {is_overlapped}, with mask id {m.tracking_id}, current mask area: {mask_area}, other mask area: {other_mask_area}, total area: {total_area}")
                            if percentage > 0.7 and distance < distance_threshold:
                                return m.tracking_id,is_touching_bottom,is_touching_top
                            isSimilar =self.Is_Similar(current_mask,m.tracking_id,x1,y1,x2,y2,frame)
                            if isSimilar is None or isSimilar:
                                print("Similar to ",m.tracking_id)
                                return m.tracking_id,is_touching_bottom,is_touching_top

                            # print("found matching mask id : ",closed_tracking , " distance is ", distance)
                            # return closed_tracking,is_touching_bottom,is_touching_top
                    #if touching_cattle_count <= 1 and distance <distance_threshold + 100: #override distance if there is one cattle touching
                        #print("Returning from touching bottom with cam_counter ",cam_counter)
                    #    return m.tracking_id, is_touching_bottom,is_touching_top
            ...
            for m in self.TRACKING_MASK_LOCATION:
                if m.cam_counter == target_counter and m.is_touching_bottom:
                    ...
                    if other_line is not None and isOverlapped:
                        ...
                        distance = self.CALCULATE_EUCLIDEAN_DISTANCE(centroid,current_point) 
                        print("centroid distance is : ",distance, " with tracking id ", m.tracking_id)
                        if distance < distance_threshold or (percentage > 0.5 and distance < distance_threshold+20): # previously allow a threshold + 20
                            mask_area = np.sum(current_mask)
                            other_mask_area = np.sum(m.mask)
                            total_area = mask_area + other_mask_area
                            if (not is_overlapped and total_area > 25000) or (is_overlapped and total_area > 38000) : #compare mask area
                                print(f"Overlapped : {is_overlapped}, current mask area: {mask_area}, other mask area: {other_mask_area}, total area: {total_area}")
                                continue
                            if percentage > 0.7 and distance < distance_threshold:
                                return m.tracking_id,is_touching_bottom,is_touching_top
    ...
    def IncreaseMissedCount(self,tracking_ids):
        if(len(self.STORED_IDS)>0): 
            self.MISSED_LEN = len(self.STORED_MISS)
        
            removed = 0
            threshold = 10
            ids = []
            for i in range(self.MISSED_LEN):
                ...
                if self.STORED_IDS[i-removed] not in tracking_ids: #didn't detected in this frame
                    ...
                    if missed>threshold: #if missed 3 frames
                        ...
                        ids.append(str(self.STORED_IDS[i-removed]))
                        print(f"deleting {self.STORED_IDS[i-removed]}, missed count is now {missed} frames")
    ...
    def GetNewTrackingIDForTrackingConflict(self,tracking_id,current_point,w,h):
        ...
        print(f'switching from {tracking_id} to {new_tracking_id} due to tracking conflict')
        #self.addNewTracking_v2(masks_np[mask_count],prev_id[1],x1,y1,x2,y2,cam_counter,w,h)
    ...
    def draw_mask(self,colored_mask,mask):
        boolean_mask = mask.astype(bool)
        colored_mask[boolean_mask] = (0, 0, 255)  # red color mask
        return colored_mask
    ...
    def isDuplicate_box(self,prev,current):
        return False
        ...
        if area / prev_area < 0.98: #check with 95%
    ...
    #    iou = area / float(prev_area + current_area - area)
        ...
        #return iou
    def IsInsideAnotherBox(self,boxes,current_box , index):
        if(index < 1): #first 
            return False
        ...
        for i in range(until):
            if self.isDuplicate_box(current_box, boxes[i]):
    ...
    def IsInMiddle(self,x1,y1,x2,y2):
        return x1 > 10 and y1 > 10 and x2 <self.boundary[0] + 300 and y2 < self.boundary[1]
    ...
    def CoreProcess(self,boxes,masks_np,h,w,frame,cam_counter):
        ...
        for box in boxes:
            ...
            if(self.IsInsideAnotherBox(boxes,box,counter)):
                #print("I am skipping")
            ...
            if True :#HOKKAIDO_ROI(x1,y1,x2,y2,h,w):  #only cow class
                ...
                #max_freq_pos = 120
                #with lock:
                    #colored_mask =draw_mask(colored_mask,mask)
                ...
                #predicted_id,tracking_id = int(prev_id[0]),int(prev_id[0]) #no identification now
                #prev_id[0] = tracking_id
    ...
    def batch_frames(self,data, file_name, is_show_image, cap= None, tracking_cap = None, isFile = False, isCam = False):
        ...
        for vdo in data:
            for key in self.distortion_keys:
                #print(vdo, ' vdo name , filter ', key)
                if key in vdo:
                    apply_correction_on[video_counter] = key
                    print("applying correction on ", key, " video_counter is :", video_counter)
                    if key in self.camera_overlapping_setting:
                        self.camera_lists[video_counter] = self.camera_overlapping_setting[key]
                    break
            video_readers = cv2.VideoCapture(vdo)
            videos.append(video_readers)
            video_counter += 1
        ...
        while ret:
            ...
            for video in videos:
                ...
                if not ret:
                    break
                if vid_counter in apply_correction_on:
                    #print('applying correction on ',key)
                    frame = self.barrel_distortion_corrector.apply_correction(frame,self.distortion_settings[apply_correction_on[vid_counter]])
                #frame = self.barrel_distortion_corrector.apply_correction(frame)
                images.append(cv2.resize(frame,(1152,648)))
                vid_counter += 1
            if manual_skip > 0:
                manual_skip -= 1
                continue
            ...
            for frame_info in all_frame_infos:
                ...
                for index in frame_info.tracked_indexes:
                    ...
                    #await draw_mask(colored_mask,masks_np[mask_count])
                    mask_count+=1
    ...
    def find_mask_center(self,mask,cam_counter):
        ...
        center_y = center_y + (self.image_height * cam_counter) #adjust for multi cam
    ...
    def begin_process(self,cam_datas,isCam,camera_number=None,lane = "Left"):
        ...
        for idx in range(number_of_videos): 
            ...
            project = f'F:/Output/runs/{self.FARMNAME}_identification/parallel_tracking_identification/May 1st Report 2025/Sumiyoshi-44/{model_name}/_{channel}'
            ...
            self.resolution = (1152,648*number_of_cams) # with number of camera because of stacking
            self.isEatingArea = False
            self.save_width,self.save_height = self.resolution
    ...
    def setup_overlapping_settings():
        ...
        overlapping_settings['5-53'] = False
    ...
    batch_size = 10 #identification batch
    total_predictions = 100
    identification_batch_size = batch_size 
    frame_skip_count = 3 #skip 3 frame read 1 frame, 5fps in 20 fps video
    ...
    REIDENTIFY_MISSED_COUNT = 20 # re identify if there are missed frame
    manual_cow_count = 0
    image_count = 1
    cow_count = 1
    ...
    #endregion added attributes
#models\December 2024\Sumiyoshi_dec16_2024_10000_iters_v9
    #model_path = "models\\November 2024\\Sumiyoshi_2024_10000_iters_v4" # for 19 nov reportSumiyoshi_oct24_2024_10000_iters_v1
    #model_path = "models\\October 2024\\Sumiyoshi_oct24_2024_10000_iters_v1" # for 19 nov report
    #model_path = "models\\December 2024\\Sumiyoshi_dec16_2024_10000_iters_v7"
    #model_path ="models\\December 2024\\Sumiyoshi_dec16_2024_10000_iters_v12" 
    ...
    predictor_path = "identification_models\\resnet101July_2025_v1_demopc"
    with open(f'{predictor_path}\\class_mapping.json', 'r') as f:
        class_names = json.load(f)
    #print(class_names)
    #identification_model_path = f"{predictor_path}\\efficientnetv2s_cattle.pth" #99.70% weight_epoch_140_weights
    
    identification_model_path = f"{predictor_path}\\best_model.pth" 
...
if __name__ == '__main__':
    ...
    inputs = [
    #     [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\1505"], 
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\1505"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\1505"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\1505"]
    # ],
    [
         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-16\\1105"], 
         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-16\\1105"],
         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-16\\1105"],
         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-16\\1105"]
    ],
#     [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\00"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\00"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\00"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\00"]
#     ],
    
#     [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\01"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\01"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\01"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\01"]
#     ],
#     [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\02"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\02"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\02"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\02"]
#     ],

# [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\03"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\03"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\03"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\03"]
#     ],
    
#     [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\04"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\04"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\04"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\04"]
#     ],
#     [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\05"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\05"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\05"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\05"]
#     ],

#  [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\06"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\06"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\06"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\06"]
#     ],
    
#     [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\07"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\07"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\07"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\07"]
#     ],
#     [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\08"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\08"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\08"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\08"]
#     ],

#  [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\09"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\09"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\09"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\09"]
#     ],
    
#     [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\10"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\10"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\10"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\10"]
#     ],
#     [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\11"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\11"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\11"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\11"]
#     ],

#  [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\12"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\12"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\12"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\12"]
#     ],
    
#     [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\13"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\13"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\13"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\13"]
#     ],
#     [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\14"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\14"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\14"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\14"]
#     ],

#  [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\15"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\15"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\15"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\15"]
#     ],
    
#     [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\16"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\16"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\16"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\16"]
#     ],
#     [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\17"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\17"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\17"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\17"]
#     ],

#  [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\18"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\18"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\18"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\18"]
#     ],
    
#     [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\19"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\19"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\19"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\19"]
#     ],
#     [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\20"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\20"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\20"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\20"]
#     ],
#     [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\21"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\21"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\21"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\21"]
#     ],
#     [
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\22"], 
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\22"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\22"],
#         ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\22"]
#     ],
    # [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-10\\23"], 
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-10\\23"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-10\\23"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-10\\23"]
    # ],
     
    # LEFT
    #["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-16\\17"], 
    #["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-16\\17"],
    #["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-16\\17"],
    #["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-16\\17"]
    # [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-16\\1105"], 
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-16\\1105"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-16\\1105"],
    ...
    # ],
    # [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-15\\20"], 
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-15\\20"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-15\\20"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-15\\20"]
    # ],
    # [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-15\\21"], 
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-15\\21"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-15\\21"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-15\\21"]
    # ],
    # [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_DX_FHD2-6-04\\2025-04-05\\12"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_DX_FHD2-6-03\\2025-04-05\\12"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_DX_FHD2-6-02\\2025-04-05\\12"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_DX_FHD2-6-01\\2025-04-05\\12"],
    # ],
    # [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_DX_FHD2-6-04\\2025-04-05\\13"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_DX_FHD2-6-03\\2025-04-05\\13"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_DX_FHD2-6-02\\2025-04-05\\13"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_DX_FHD2-6-01\\2025-04-05\\13"],
    # ],
    # [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_DX_FHD2-6-04\\2025-04-05\\14"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_DX_FHD2-6-03\\2025-04-05\\14"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_DX_FHD2-6-02\\2025-04-05\\14"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_DX_FHD2-6-01\\2025-04-05\\14"],
    # ],
    # [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_DX_FHD2-6-04\\2025-04-05\\15"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_DX_FHD2-6-03\\2025-04-05\\15"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_DX_FHD2-6-02\\2025-04-05\\15"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_DX_FHD2-6-01\\2025-04-05\\15"],
    # ],
    # [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_DX_FHD2-6-04\\2025-04-05\\16"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_DX_FHD2-6-03\\2025-04-05\\16"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_DX_FHD2-6-02\\2025-04-05\\16"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_DX_FHD2-6-01\\2025-04-05\\16"],
    # ],
    # [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_DX_FHD2-6-04\\2025-04-05\\17"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_DX_FHD2-6-03\\2025-04-05\\17"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_DX_FHD2-6-02\\2025-04-05\\17"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_DX_FHD2-6-01\\2025-04-05\\17"],
    # ],
    # [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_DX_FHD2-6-04\\2025-04-05\\18"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_DX_FHD2-6-03\\2025-04-05\\18"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_DX_FHD2-6-02\\2025-04-05\\18"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_DX_FHD2-6-01\\2025-04-05\\18"],
    # ],
    # [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_DX_FHD2-6-04\\2025-04-05\\19"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_DX_FHD2-6-03\\2025-04-05\\19"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_DX_FHD2-6-02\\2025-04-05\\19"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_DX_FHD2-6-01\\2025-04-05\\19"],
    # ],
    # [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_DX_FHD2-6-04\\2025-04-05\\20"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_DX_FHD2-6-03\\2025-04-05\\20"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_DX_FHD2-6-02\\2025-04-05\\20"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_DX_FHD2-6-01\\2025-04-05\\20"],
    # ],
    
    # [
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_Camera 005\\2025-03-31\\09"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_Camera 004\\2025-03-31\\09"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_Camera 003\\2025-03-31\\09"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_Camera 002\\2025-03-31\\09"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_Camera 001\\2025-03-31\\09"]
    # ],
    # [
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_Camera 005\\2025-03-31\\10"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_Camera 004\\2025-03-31\\10"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_Camera 003\\2025-03-31\\10"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_Camera 002\\2025-03-31\\10"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_Camera 001\\2025-03-31\\10"]
    # ],
    # [
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_Camera 005\\2025-03-31\\11"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_Camera 004\\2025-03-31\\11"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_Camera 003\\2025-03-31\\11"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_Camera 002\\2025-03-31\\11"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_Camera 001\\2025-03-31\\11"]
    # ],
    # [
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_Camera 005\\2025-03-31\\12"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_Camera 004\\2025-03-31\\12"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_Camera 003\\2025-03-31\\12"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_Camera 002\\2025-03-31\\12"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_Camera 001\\2025-03-31\\12"]
    # ],
    # [
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_Camera 005\\2025-03-31\\13"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_Camera 004\\2025-03-31\\13"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_Camera 003\\2025-03-31\\13"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_Camera 002\\2025-03-31\\13"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_Camera 001\\2025-03-31\\13"]
    # ],
    # [
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_Camera 005\\2025-03-31\\14"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_Camera 004\\2025-03-31\\14"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_Camera 003\\2025-03-31\\14"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_Camera 002\\2025-03-31\\14"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_Camera 001\\2025-03-31\\14"]
    # ],
    # [
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_Camera 005\\2025-03-31\\15"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_Camera 004\\2025-03-31\\15"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_Camera 003\\2025-03-31\\15"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_Camera 002\\2025-03-31\\15"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_Camera 001\\2025-03-31\\15"]
    # ],
    # [
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_Camera 005\\2025-03-31\\16"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_Camera 004\\2025-03-31\\16"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_Camera 003\\2025-03-31\\16"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_Camera 002\\2025-03-31\\16"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_Camera 001\\2025-03-31\\16"]
    # ],
    # [
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_Camera 005\\2025-03-31\\17"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_Camera 004\\2025-03-31\\17"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_Camera 003\\2025-03-31\\17"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_Camera 002\\2025-03-31\\17"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_Camera 001\\2025-03-31\\17"]
    # ],
    # [
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_Camera 005\\2025-03-31\\18"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_Camera 004\\2025-03-31\\18"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_Camera 003\\2025-03-31\\18"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_Camera 002\\2025-03-31\\18"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_Camera 001\\2025-03-31\\18"]
    # ],
    #  [
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_Camera 005\\2025-03-31\\19"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_Camera 004\\2025-03-31\\19"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_Camera 003\\2025-03-31\\19"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_Camera 002\\2025-03-31\\19"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_Camera 001\\2025-03-31\\19"]
    # ],
    #  [
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_Camera 005\\2025-03-31\\20"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_Camera 004\\2025-03-31\\20"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_Camera 003\\2025-03-31\\20"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_Camera 002\\2025-03-31\\20"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_Camera 001\\2025-03-31\\20"]
    # ],
     
    # [
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_Camera 005\\2025-03-31\\22"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_Camera 004\\2025-03-31\\22"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_Camera 003\\2025-03-31\\22"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_Camera 002\\2025-03-31\\22"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_Camera 001\\2025-03-31\\22"]
    # ],
    # [
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_Camera 005\\2025-03-31\\23"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_Camera 004\\2025-03-31\\23"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_Camera 003\\2025-03-31\\23"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_Camera 002\\2025-03-31\\23"],
    #     ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_Camera 001\\2025-03-31\\23"]
    # ],

    # [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-16\\16",],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-16\\16"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-16\\16"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-16\\16"]
    # ],
    ...
    #  [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-16\\18",],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-16\\18"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-16\\18"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-16\\18"]
    # ],

    #  [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-16\\19",],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-16\\19"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-16\\19"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-16\\19"]
    # ],

    #  [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-16\\20",],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-16\\20"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-16\\20"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-16\\20"]
    # ],

    # [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-16\\21",],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-16\\21"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-16\\21"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-16\\21"]
    # ],

    # [
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-54\\2025-04-16\\22",],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_DX-FHD-5-53\\2025-04-16\\22"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch007_DX-FHD-5-52\\2025-04-16\\22"],
    #     ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch006_DX-FHD-5-51\\2025-04-16\\22"]
    # ],
    # MIDDLE 
    #["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_DX_FHD2-6-04\\2025-04-02\\1535"],
    #["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_DX_FHD2-6-03\\2025-04-02\\1535"]
    # ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_DX_FHD2-6-02\\2025-04-02\\1110"],
    # ["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_DX_FHD2-6-01\\2025-04-02\\1110"],
    
    #["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_DX_FHD2-6-04\\2025-04-02\\1535"],
    #["F:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_DX_FHD2-6-03\\2025-04-02\\1535"]

    # RIGHT
    # ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch005_Camera 005\\2025-03-31\\1110"],
    # ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch004_Camera 004\\2025-03-31\\1110"],
    # ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch003_Camera 003\\2025-03-31\\1110"],
    # ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch002_Camera 002\\2025-03-31\\1110"],
    # ["E:\\Nyi Zaw Aung\\815_CowDataChecking\\Sumiyoshii\\Standard_Format\\ALL_Channel_One_Day\\Ch001_Camera 001\\2025-03-31\\1110"]
    ]
    
    #allCams = [cam4,cam3,cam2,cam1] #put from bottom to top like 604 603 602 601 cameras
    
    # filelist = [r"C:\Users\choni\Pictures\5-54.png",
    #         r"C:\Users\choni\Pictures\5-53.png",
    #         r"C:\Users\choni\Pictures\5-52.png",
    #         r"C:\Users\choni\Pictures\5-51.png"]
    # for file in filelist:
    #     img = cv2.imread(file)
    #     images.append(cv2.resize(img,(1152,648))    )
    # #predictions = list(pred(filelist[0:3]))
    # predictions = list(begin_identification.stupid_detector(images))
    ...
    #     boxes = instances.pred_boxes.tensor.numpy()
    #     masks_np = instances.pred_masks.numpy()
    #     draw_bounding_boxes_out(images[counter], boxes)
    #     draw_masks_out(images[counter], masks_np)
    #     #cv2.imshow("result", cv2.resize(images[counter],(1152,648)))
    #     #cv2.waitKey(0)
    #     #cv2.destroyAllWindows()
    #     #for box in result:
    #     #    print(result)
    #     #savepath = rf"C:\Users\choni\Pictures\Merged image\individual_{counter}.png"
    #     #cv2.imwrite(savepath, images[counter])
    #     counter +=1
    # stack_image = begin_identification.stack_image_from_bottom_to_top(images)
    # cv2.imshow('stacked image begin process',cv2.resize(stack_image,(460,1036)))
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
